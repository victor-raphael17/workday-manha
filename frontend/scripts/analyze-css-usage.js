...
async function analyzeCSS() {
  try {
    ...
    const result = await new PurgeCSS().purge({
      ...
      css: [toGlob("assets/css/app.css"), toGlob("assets/css/theme.css")],
      safelist: {
        standard: [
          /^modal-/,
          /^collapse/,
    ...
    console.log(
      `   • CSS original:       ${originalSize} bytes (${originalLines} linhas)`
    );
    console.log(
      `   • CSS utilizado:      ${purgecssSize} bytes (${purgecssLines} linhas)`
    );
    console.log(
      `   • CSS não utilizado:  ${removedSize} bytes (${removedLines} linhas)`
    );
